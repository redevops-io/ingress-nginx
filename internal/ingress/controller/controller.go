...
func (n *NGINXController) CheckIngress(ing *networking.Ingress) error {
	...
	// Always enforce pathType validation. The StrictValidatePathType switch is now
	// only used to warn administrators who still expect the old, weaker behavior.
	if err := inspector.ValidatePathType(ing); err != nil {
		if !cfg.StrictValidatePathType {
			klog.Warningf("ingress %s/%s rejected: strict path validation can no longer be disabled; remove 'strict-validate-path-type' from the configmap", ing.GetNamespace(), ing.GetName())
		}
	...
	for key, value := range ing.ObjectMeta.GetAnnotations() {
		...
		if strings.HasSuffix(key, "-snippet") {
			return fmt.Errorf("%s annotation cannot be used. Snippet directives are permanently disabled in this distribution", key)
		}
	}
...
func dropSnippetDirectives(anns *annotations.Ingress, ingKey string) {
	if anns != nil {
		if anns.ConfigurationSnippet != "" {
			klog.V(3).Infof("Ingress %q tried to use configuration-snippet and the annotation is forbidden. Dropping it", ingKey)
			anns.ConfigurationSnippet = ""
		}
		if anns.ServerSnippet != "" {
			klog.V(3).Infof("Ingress %q tried to use server-snippet and the annotation is forbidden. Dropping it", ingKey)
			anns.ServerSnippet = ""
		}

		if anns.ModSecurity.Snippet != "" {
			klog.V(3).Infof("Ingress %q tried to use modsecurity-snippet and the annotation is forbidden. Dropping it", ingKey)
			anns.ModSecurity.Snippet = ""
		}

		if anns.ExternalAuth.AuthSnippet != "" {
			klog.V(3).Infof("Ingress %q tried to use auth-snippet and the annotation is forbidden. Dropping it", ingKey)
			anns.ExternalAuth.AuthSnippet = ""
		}

		if anns.StreamSnippet != "" {
			klog.V(3).Infof("Ingress %q tried to use stream-snippet and the annotation is forbidden. Dropping it", ingKey)
			anns.StreamSnippet = ""
		}
	}
}
